...
rxvt_drawable &rxvt_screen::scratch_drawable(int w, int h)
{
  if (!scratch_area || w > scratch_w || h > scratch_h) {
    ...
    Pixmap pm = XCreatePixmap(dpy, RootWindowOfScreen(ScreenOfDisplay(dpy, display->screen)), scratch_w = w, scratch_h = h, depth);
...
void rxvt_screen::set(rxvt_display *disp)
{
  display = disp;
  dpy = disp->dpy;
...
void rxvt_screen::select_visual(int id)
{
  XVisualInfo vinfo;
  vinfo.visualid = id;
  int n;
...
void rxvt_screen::select_depth(int bitdepth)
{
  XVisualInfo vinfo;
...
rxvt_display::rxvt_display(const char *id) :
    refcounted(id), selection_owner(0), clipboard_owner(0)
{
...
rxvt_display::get_resources(bool refresh)
{
  ...
  if (refresh) {
    ...
    if (XGetWindowProperty(dpy, RootWindow(dpy, 0), XA_RESOURCE_MANAGER, 0L, 100000000L, False, XA_STRING, &actual_type, &actual_format, &nitems, &nremaining, (unsigned char **)&val) == Success && actual_type == XA_STRING && actual_format == 8)
      displayResource = val;
    else {
      displayResource = 0;
...
bool rxvt_display::ref_init()
{
  ...
  blank_cursor = XCreateGlyphCursor(dpy, f, f, ' ', ' ', &blackcolour, &blackcolour);
  XUnloadFont(dpy, f);
#endif
...
void rxvt_display::im_change_check()
{
  ...
  if (XGetWindowProperty(dpy, root, xa[XA_XIM_SERVERS], 0L, 1000000L, False, XA_ATOM, &actual_type, &actual_format, &nitems, &bytes_after, (unsigned char **)&atoms) != Success)
    return;
  ...
  while (XEventsQueued(dpy, QueuedAfterFlush))
    do {
      ...
      if (!XFilterEvent(&xev, None)) {
        if (xev.type == PropertyNotify && xev.xany.window == root && xev.xproperty.atom == xa[XA_XIM_SERVERS])
          im_change_check();
#endif
...
rxvt_display::get_pixmap_property(Atom property)
{
  ...
  Atom atype;
  unsigned char *prop;
  int result = XGetWindowProperty(dpy, root, property, 0L, 1L, False, XA_PIXMAP, &atype, &aformat, &nitems, &bytes_after, &prop);
  if (result == Success) {
    if (atype == XA_PIXMAP)
      pixmap = *(Pixmap *)prop;
    XFree(prop);
...
bool rxvt_color::alloc(rxvt_screen *screen, const rgba &color)
{
  ...
  if ((screen->visual->c_class == TrueColor) && (format = XRenderFindVisualFormat(screen->dpy, screen->visual))) {
...
bool rxvt_color::set(rxvt_screen *screen, const char *name)
{
  rgba c;
  char eos;
  int skip = 0;
...
bool rxvt_color::set(rxvt_screen *screen, const rgba &color)
{
  ...
  if (!got && screen->visual->c_class == PseudoColor && cmap_size < 4096) {
...
void rxvt_color::get(rgba &color) const
{
#if XFT
...
void rxvt_color::get(XColor &color) const
{
  rgba c;
  get(c);
...
void rxvt_color::free(rxvt_screen *screen)
{
  if (screen->visual->c_class == TrueColor)
...
void rxvt_color::fade(rxvt_screen *screen, int percent, rxvt_color &result, const rgba &to)
{
  rgba c;
  get(c);
...
void rxvt_selection::finish(char *data, unsigned int len)
{
  if (!cb_sv) {
    if (data)
      term->paste(data, len);
...
bool rxvt_selection::request(Atom target, int selnum)
{
  ...
  if (XGetSelectionOwner(display->dpy, sel) != None) {
    XConvertSelection(display->dpy, sel, target, request_prop, request_win, request_time);
...
void rxvt_selection::handle_selection(Window win, Atom prop, bool delete_prop)
{
  ...
  if (XGetWindowProperty(dpy, win, prop, 0, 0x1fffffff, delete_prop, AnyPropertyType, &ct.encoding, &ct.format, &ct.nitems, &bytes_after, &ct.value) != Success) {
    ct.value = 0;
    goto bailout;
  }
...
void rxvt_selection::timer_cb(ev::timer &w, int revents)
{
  if (selection_wait == Sel_incr)
...
void rxvt_selection::x_cb(XEvent &xev)
{
  switch (xev.type) {
    case PropertyNotify:
      if (selection_wait == Sel_incr && xev.xproperty.atom == request_prop && xev.xproperty.state == PropertyNewValue)
        handle_selection(xev.xproperty.window, xev.xproperty.atom, true);
      break;

    case SelectionNotify:
      if (selection_wait == Sel_normal && xev.xselection.time == request_time) {
        timer_ev.stop();
