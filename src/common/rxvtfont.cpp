...
#define OVERLAP_OK(w, wcw, prop) ((w) <= ((prop)->slant >= rxvt_fontprop::italic ? ((prop)->width * (wcw)*MAX_OVERLAP_ITALIC + 7) >> 3 : ((prop)->width * (wcw)*MAX_OVERLAP_ROMAN + 7) >> 3))
...
void rxvt_font::set_name(char *name_)
{
  if (name == name_)
    return;
...
void rxvt_font::clear_rect(rxvt_drawable &d, int x, int y, int w, int h, int color) const
{
  dTermDisplay;
  dTermGC;
...
#ifdef HAVE_IMG
    if (term->bg_img && !term->pix_colors[color].is_opaque() && ((dst = XftDrawPicture(d)))) {
      XClearArea(disp, d, x, y, w, h, false);
...
struct rxvt_font_default : rxvt_font {
  ...
  bool has_char(unicode_t unicode, const rxvt_fontprop *prop, bool &careful) const
  {
    ...
    if (unicode >= 0x2500 && unicode <= 0x259f && !term->option(Opt_skipBuiltinGlyphs))
      return true;
#endif
  ...
  void draw(rxvt_drawable &d, int x, int y, const text_t *text, int len, int fg, int bg);
};

void rxvt_font_default::draw(rxvt_drawable &d, int x, int y, const text_t *text, int len, int fg, int bg)
{
  dTermDisplay;
  dTermGC;
        ...
        switch (op) {
          ...
          case 1:  // rectangle, possibly stippled
            if (a) {
              ...
              XChangeGC(disp, gc, GCFillStyle | GCStipple | GCTileStipXOrigin | GCTileStipYOrigin, &gcv);
            }
            ...
            break;
          case 2:  // arc
            XDrawArc(disp, d, gc, x1 - W / 2, y1 - H / 2, W - 1, H - 1, (a - 1) * 90 * 64, (b - 1) * 90 * 64);
            break;
        }
      }
    }
    ...
    if (0)
      ...
      *chrs = cc->c1;
      ...
      switch (t) {
        ...
        default:
          XDrawRectangle(disp, d, gc, x + 2, y + 2, fwidth - 4, term->fheight - 4);
      }
...
bool rxvt_font_x11::set_properties(rxvt_fontprop &p, int height, const char *weight, const char *slant, int avgwidth)
{
...
bool rxvt_font_x11::set_properties(rxvt_fontprop &p, XFontStruct *f)
{
...
bool rxvt_font_x11::set_properties(rxvt_fontprop &p, const char *name)
{
  dTermDisplay;
  int slashes = 0;
  const char *comp[13];
  ...
  if (slashes >= 13 && (*comp[6] >= '1' && *comp[6] <= '9') && (*comp[11] >= '0' && *comp[11] <= '9'))
...
bool rxvt_font_x11::load(const rxvt_fontprop &prop, bool force_prop)
{
  dTermDisplay;
  ...
  if (force_prop) {
    ...
    if (prop.weight != rxvt_fontprop::unset) {
      replace_field(&fname, name, 2, 0, prop.weight < rxvt_fontprop::bold ? "medium" : "bold");
      set_name(fname);
    }
    ...
    if (prop.slant != rxvt_fontprop::unset) {
      replace_field(
        ...
        3,
        0,
        prop.slant < rxvt_fontprop::italic ? "r" : "i"
      );  // TODO: handle "o"blique, too
      set_name(fname);
    }
  }

  sprintf(field_str, "%d", prop.height == rxvt_fontprop::unset ? 0 : prop.height);
  ...
  for (int i = 0; i < count; i++) {
    ...
    if (!set_properties(p, fname)
        ...
        || (prop.height != rxvt_fontprop::unset && p.height > prop.height)) {
      free(fname);
      continue;
    }
  ...
  for (;;) {
    ...
    if (!best->name || !(f = XLoadQueryFont(disp, best->name)))
      break;
    ...
    if (prop.height == rxvt_fontprop::unset || height <= prop.height)
...
bool rxvt_font_x11::has_char(unicode_t unicode, const rxvt_fontprop *prop, bool &careful) const
{
  careful = false;
  ...
  if (encm) {
    ...
    if (byte1 < f->min_byte1 || byte1 > f->max_byte1 || byte2 < f->min_char_or_byte2 || byte2 > f->max_char_or_byte2)
      return false;
  ...
  if (xcs->lbearing == 0 && xcs->rbearing == 0 && xcs->width == 0 && xcs->ascent == 0 && xcs->descent == 0)
    return false;
...
void rxvt_font_x11::draw(rxvt_drawable &d, int x, int y, const text_t *text, int len, int fg, int bg)
{
...
bool rxvt_font_xft::load(const rxvt_fontprop &prop, bool force_prop)
{
  dTermDisplay;
  ...
  if (prop.height != rxvt_fontprop::unset && (FcPatternGet(p, FC_PIXEL_SIZE, 0, &v) != FcResultMatch && FcPatternGet(p, FC_SIZE, 0, &v) != FcResultMatch))
    FcPatternAddInteger(p, FC_PIXEL_SIZE, prop.height);

  if (prop.weight != rxvt_fontprop::unset && (force_prop || FcPatternGet(p, FC_WEIGHT, 0, &v) != FcResultMatch))
    FcPatternAddInteger(p, FC_WEIGHT, prop.weight);

  if (prop.slant != rxvt_fontprop::unset && (force_prop || FcPatternGet(p, FC_SLANT, 0, &v) != FcResultMatch))
  ...
  for (;;) {
    ...
    for (uint16_t *t = extent_test_chars; t < extent_test_chars + ecb_array_length(extent_test_chars); t++) {
      ...
      if (cs != CS_UNICODE && ch > 0x100 && FROM_UNICODE(cs, ch) == NOCHAR)
        continue;
    ...
    if (prop.height == rxvt_fontprop::unset || (height <= prop.height && glheight <= prop.height) || height <= 2 || !scalable)
      break;
...
bool rxvt_font_xft::has_char(unicode_t unicode, const rxvt_fontprop *prop, bool &careful) const
{
  careful = false;
...
void rxvt_font_xft::draw(rxvt_drawable &d, int x, int y, const text_t *text, int len, int fg, int bg)
{
...
// handle non-bmp chars when text_t is 16 bit
  ...
  if (buffered) {
    if (ep != enc) {
      ...
      if (term->bg_img && (bg == Color_transparent || bg == Color_bg || (bg >= 0 && !term->pix_colors[bg].is_opaque() && ((dst = XftDrawPicture(d2)))))) {
        int src_x = x, src_y = y;
        ...
        if (term->bg_img->w >= src_x + w && term->bg_img->h >= src_y + h) {
          XCopyArea(disp, term->bg_img->pm, d2, gc, src_x, src_y, w, h, 0, 0);
        } else {
          XGCValues gcv;
          ...
          XChangeGC(disp, gc, GCTile | GCTileStipXOrigin | GCTileStipYOrigin | GCFillStyle, &gcv);
...
{
  prop.width = prop.height = prop.ascent = prop.weight = prop.slant = rxvt_fontprop::unset;
  force_prop = false;
...
void rxvt_fontset::prepare_font(rxvt_font *font, codeset cs)
{
  font->set_term(term);
...
void rxvt_fontset::push_font(rxvt_font *font)
{
...
void rxvt_fontset::add_fonts(const char *desc)
{
  if (desc) {
    char buf[512];
    const char *end;
...
bool rxvt_fontset::realize_font(int i)
{
  if (i < 0 || i >= fonts.size())
    return false;
...
bool rxvt_fontset::populate(const char *desc)
{
  clear();
...
int rxvt_fontset::find_font(const char *name) const
{
...
int rxvt_fontset::find_font_idx(unicode_t unicode)
{
