  ...
  for (i = 0, ifr = ifc.ifc_req;
    ...
    if (ioctl(skfd, SIOCGIFADDR, &ifr2) >= 0) {
      ...
      if (addr && addr != 0x7F000001) {
        ...
        sprintf(rval, "%d.%d.%d.%d%s", (int)((addr >> 030) & 0xFF), (int)((addr >> 020) & 0xFF), (int)((addr >> 010) & 0xFF), (int)(addr & 0xFF), colon);
        break;
      }
    }
  }
...
/*----------------------------------------------------------------------*/
void rxvt_term::init(stringvec *argv, stringvec *envv)
{
  argv->push_back(0);
  envv->push_back(0);
...
void rxvt_term::init2(int argc, const char *const *argv)
{
  ...
  if (rs[Rs_embed]) {
    ...
    XChangeProperty(dpy, parent, xa[XA_XEMBED_INFO], xa[XA_XEMBED_INFO], 32, PropModeReplace, (unsigned char *)&info, 2);
  }
#endif
...
/*----------------------------------------------------------------------*/
void rxvt_term::set_locale(const char *locale)
{
  set_environ(env);
...
/*----------------------------------------------------------------------*/
void rxvt_term::init_command(const char *const *argv)
{
  /*
...
void rxvt_term::color_aliases(int idx)
{
...
void rxvt_term::get_ourmods()
{
  ...
  if (rsmod && strcasecmp(rsmod, "mod1") >= 0 && strcasecmp(rsmod, "mod5") <= 0)
...
void rxvt_term::set_icon(const char *file)
{
  ...
  if (long *buffer = (long *)malloc((2 + w * h) * sizeof(long))) {
    ...
    XChangeProperty(dpy, parent, xa[XA_NET_WM_ICON], XA_CARDINAL, 32, PropModeReplace, (const unsigned char *)buffer, 2 + w * h);
    free(buffer);
  } else
...
void rxvt_term::create_windows(int argc, const char *const *argv)
{
  ...
  top = XCreateWindow(dpy, parent, szHint.x, szHint.y, szHint.width, szHint.height, ext_bwidth, depth, InputOutput, visual, CWColormap | CWBackPixel | CWBorderPixel | CWOverrideRedirect, &attributes);
  ...
  XmbSetWMProperties(dpy, top, NULL, NULL, (char **)argv, argc, &szHint, &wmHint, &classHint);
#if ENABLE_EWMH
  /*
   * set up icon hint
...
#if ENABLE_FRILLS
  if (mwmhints.flags)
    XChangeProperty(dpy, top, xa[XA_MOTIF_WM_HINTS], xa[XA_MOTIF_WM_HINTS], 32, PropModeReplace, (unsigned char *)&mwmhints, PROP_MWM_HINTS_ELEMENTS);
#endif
  ...
  XChangeProperty(dpy, top, xa[XA_NET_WM_PID], XA_CARDINAL, 32, PropModeReplace, (unsigned char *)&pid, 1);

  // _NET_WM_WINDOW_TYPE is NORMAL, which is the default
#endif

  XSelectInput(dpy, top, KeyPressMask
#if (MOUSE_WHEEL && MOUSE_SLIP_WHEELING) || ENABLE_FRILLS || ISO_14755
                           | KeyReleaseMask
#endif
                           | FocusChangeMask | VisibilityChangeMask | ExposureMask | StructureNotifyMask);
  ...
  /* the vt window */
  vt = XCreateSimpleWindow(dpy, top, window_vt_x, window_vt_y, vt_width, vt_height, 0, pix_colors_focused[Color_fg], pix_colors_focused[Color_bg]);
  ...
  gc = XCreateGC(dpy, vt, GCForeground | GCBackground | GCGraphicsExposures, &gcvalue);
 ...
 * the slave.
 */
void rxvt_term::run_command(const char *const *argv)
{
#if ENABLE_FRILLS
  if (rs[Rs_pty_fd]) {
 ...
 * returns are fatal
 */
int rxvt_term::run_child(const char *const *argv)
{
  char *login;
