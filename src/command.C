...
rxvt_term::iso14755_51(unicode_t ch, rend_t r, int x, int y, int y2)
{
  ...
  sprintf(attr, "%08x = fg %d bg %d%s%s%s%s%s%s", (int)r, fgcolor_of(r), bgcolor_of(r), r & RS_Bold ? " bold" : "", r & RS_Italic ? " italic" : "", r & RS_Blink ? " blink" : "", r & RS_RVid ? " rvid" : "", r & RS_Uline ? " uline" : "", r & RS_Careful ? " careful" : "");
...
hex_keyval(XKeyEvent &ev)
{
  ...
  for (int index = 0; index < 8; index++) {
    ...
    if (k >= XK_KP_0 && k <= XK_KP_9)
      return k - XK_KP_0;
    else if (k >= XK_0 && k <= XK_9)
      return k - XK_0;
    else if (k >= XK_a && k <= XK_f)
      return k - XK_a + 10;
    else if (k >= XK_A && k <= XK_F)
      return k - XK_A + 10;
  }
...
rxvt_term::key_press(XKeyEvent &ev)
{
  ...
  if (Input_Context) {
...
#endif
    {
      ...
      len = XwcLookupString(Input_Context, &ev, wkbuf, KBUFSZ, &keysym, &status_return);
  ...
  if (keysym != NoSymbol) {
    ...
    if (iso14755buf & (ISO_14755_STARTED | ISO_14755_51)) {
      ...
      if (iso14755buf & ISO_14755_51 && (keysym == XK_space || keysym == XK_KP_Space || keysym == XK_Return || keysym == XK_KP_Enter)) {
        commit_iso14755();
    ...
    } else if (option(Opt_iso14755) && ((ctrl && (keysym == XK_Shift_L || keysym == XK_Shift_R)) || (shft && (keysym == XK_Control_L || keysym == XK_Control_R))))
...
rxvt_term::key_release(XKeyEvent &ev)
{
  ...
  if (iso14755buf)
    if (iso14755buf & ISO_14755_52) {
      ...
      // first: controls
      if ((ev.state & ControlMask) && ((keysym >= 0x40 && keysym <= 0x5f) || (keysym >= 0x61 && keysym <= 0x7f))) {
...
#ifdef CURSOR_BLINK
void rxvt_term::cursor_blink_reset()
{
  if (!focus)
    return;
...
void rxvt_term::cursor_blink_cb(ev::timer &w, int revents)
{
...
void rxvt_term::cont_scroll_cb(ev::timer &w, int revents)
{
  if ((scrollBar.state == SB_STATE_UP || scrollBar.state == SB_STATE_DOWN) && scr_page(scrollBar.state == SB_STATE_UP ? UP : DN, 1)) {
    want_refresh = 1;
    refresh_check();
  } else
    w.stop();
...
void rxvt_term::sel_scroll_cb(ev::timer &w, int revents)
{
...
#if defined(MOUSE_WHEEL) && defined(MOUSE_SLIP_WHEELING)
void rxvt_term::slip_wheel_cb(ev::timer &w, int revents)
{
...
void rxvt_term::cmdbuf_append(const char *str, size_t count)
{
  cmdbuf_reify();
...
void rxvt_term::mouse_report(XButtonEvent &ev)
{
  ...
  if (priv_modes & PrivMode_ExtMouseSGR)
    tt_printf("\033[<%d;%d;%d%c", button_number + state, x, y, release ? 'm' : 'M');
  else if (priv_modes & PrivMode_ExtMouseUrxvt)
    tt_printf("\033[%d;%d;%dM", code, x, y);
  else if (priv_modes & PrivMode_ExtMouseUTF8)
    if (code < 128)
      tt_printf("\033[M%c%lc%lc", code, wint_t(32 + x), wint_t(32 + y));
    else
      tt_printf("\033[M%c%c%lc%lc", 0xc0 + (code >> 6), 0x80 + (code & 0x3f), wint_t(32 + x), wint_t(32 + y));
  else
#endif
    tt_printf("\033[M%c%c%c", code, 32 + x, 32 + y);
}
...
rxvt_term::x_cb(XEvent &ev)
{
  ...
  switch (ev.type) {
    ...
    case FocusIn:
      if (ev.xfocus.detail != NotifyInferior && ev.xfocus.detail != NotifyPointer && ev.xfocus.mode != NotifyGrab)
        focus_in();
      break;
    ...
    case Expose:
      if (ev.xany.window == vt) {
        do {
          scr_expose(ev.xexpose.x, ev.xexpose.y, ev.xexpose.width, ev.xexpose.height, false);
    ...
    case MotionNotify:
      ...
      if (ev.xany.window == vt) {
        if (SHOULD_INVOKE(HOOK_MOTION_NOTIFY) && HOOK_INVOKE((this, HOOK_MOTION_NOTIFY, DT_XEVENT, &ev, DT_END)))
          ...
          XQueryPointer(dpy, vt, &unused_root, &unused_child, &unused_root_x, &unused_root_y, &ev.xbutton.x, &ev.xbutton.y, &ev.xbutton.state);
...
#endif
            selection_extend(ev.xbutton.x, ev.xbutton.y, ev.xbutton.state & Button3Mask ? 2 : 0);

#ifdef SELECTION_SCROLLING
            if (ev.xbutton.y < int_bwidth || Pixel2Row(ev.xbutton.y) > (nrow - 1)) {
              ...
              scroll_selection_lines = Pixel2Height(dist)
                                       ...
                                       + 1;
              min_it(scroll_selection_lines, SELECTION_SCROLL_MAX_LINES);
        ...
        XQueryPointer(dpy, scrollBar.win, &unused_root, &unused_child, &unused_root_x, &unused_root_y, &ev.xbutton.x, &ev.xbutton.y, &unused_mask);
        scr_move_to(scrollBar.position(ev.xbutton.y) - csrO, scrollBar.size());
        want_refresh = 1;
        scrollBar.show(1);
      }
      break;
...
void rxvt_term::button_release(XButtonEvent &ev)
{
  int reportmode = 0;
  ...
  if (ev.window == vt) {
    ...
    if (reportmode) {
      ...
      if (MEvent.button != AnyButton && (ev.button != MEvent.button || (ev.time - MEvent.time > multiClickTime / 2))) {
        MEvent.clicks = 0;
        MEvent.button = ev.button;
        mouse_report(ev);
      }
     ...
     */
    if (priv_modes & PrivMode_mouse_report && bypass_keystate && ev.button == Button1 && MEvent.clicks <= 1)
...
void rxvt_term::process_window_ops(const int *args, unsigned int nargs)
{
  ...
  switch (args[0]) {
    ...
    case 8: /* set size (chars) */
      set_widthheight((unsigned int)(args[2] * fwidth), (unsigned int)(args[1] * fheight));
      break;

    // case 9: NYI, TODO, restore maximized window or maximize window
    default:
      if (args[0] >= 24) /* set height (chars) */
        set_widthheight((unsigned int)vt_width, (unsigned int)(args[1] * fheight));
      break;
    ...
    case 13: /* report window position */
      XGetWindowAttributes(dpy, parent, &wattr);
      XTranslateCoordinates(dpy, parent, wattr.root, -wattr.border_width, -wattr.border_width, &x, &y, &wdummy);
...
static unsigned int
colorcube_index(unsigned int idx_r, unsigned int idx_g, unsigned int idx_b)
{
  assert(idx_r < Red_levels);
  assert(idx_g < Green_levels);
  assert(idx_b < Blue_levels);

  return idx_r * Blue_levels * Green_levels + idx_g * Blue_levels + idx_b;
}
...
void rxvt_term::process_color_seq(int report, int color, const char *str, string_term &st)
{
  if (str[0] == '?' && !str[1]) {
    rgba c;
 ...
 */
void rxvt_term::process_xterm_seq(int op, char *str, string_term &st)
{
  int color;
  char *buf, *name;
  ...
  switch (op) {
    ...
    case XTerm_property:
      if (str[0] == '?') {
        ...
        if (prop && XGetWindowProperty(dpy, parent, prop, 0, 1 << 16, 0, AnyPropertyType, &actual_type, &actual_format, &nitems, &bytes_after, &value) == Success && actual_type != None && actual_format == 8)
        ...
        if (eq) {
          *eq = 0;
          ...
          XDeleteProperty(dpy, parent, display->atom(str));
      }
      break;
...
#endif
      if (query)
        tt_printf("\33]%d;%-.250s%s", saveop, option(Opt_insecure) && fontset[op - URxvt_font]->fontdesc ? fontset[op - URxvt_font]->fontdesc : "", st.v);
      else {
    ...
    case URxvt_version:
      if (query)
        tt_printf("\33]%d;rxvt-unicode;%-.20s;%c;%c%s", op, rs[Rs_name], VERSION[0], VERSION[2], st.v);
      break;

#if !ENABLE_MINIMAL
    case URxvt_cellinfo:
      if (query)
        tt_printf("\33]%d;%d;%d;%d%s", saveop, fwidth, fheight, fbase, st.v);
      break;
...
void rxvt_term::process_terminal_mode(int mode, int priv ecb_unused, unsigned int nargs, const int *arg)
{
  unsigned int i, j;
  int state;
...
void rxvt_term::set_cursor_style(int style)
{
...
void rxvt_term::tt_write_(const char *data, unsigned int len)
{
  if (pty->pty < 0)
    return;
