...
bool rxvt_set_locale(const char *locale) noexcept
{
...
void rxvt_push_locale(const char *locale) noexcept
{
  strcpy(savelocale, curlocale);
  rxvt_set_locale(locale);
}

void rxvt_pop_locale() noexcept
{
...
rxvt_term::rxvt_term()
{
  ...
  cursor_blink_ev.set(0., CURSOR_BLINK_INTERVAL);
#endif
#ifdef TEXT_BLINK
  text_blink_ev.set<rxvt_term, &rxvt_term::text_blink_cb>(this);
  text_blink_ev.set(0., TEXT_BLINK_INTERVAL);
#endif
...
void rxvt_term::destroy_cb(ev::idle &w, int revents)
{
  make_current();
...
void rxvt_term::set_option(uint8_t opt, bool set) noexcept
{
  if (!opt)
    return;
...
print_x_error(Display *dpy, XErrorEvent *event)
{
  ...
  snprintf(buffer, BUFSIZ, "+ %s\n", mesg);
  rxvt_warn(buffer, event->request_code);
  ...
  if (event->request_code >= 128) {
    ...
    if (!nexts) {
      ...
      XGetErrorDatabaseText(dpy, "XlibMessage", "MinorCode", "Request Minor code %d", mesg, BUFSIZ);
      snprintf(buffer, BUFSIZ, "+ %s\n", mesg);
      rxvt_warn(buffer, event->minor_code);
  ...
  if (event->error_code == BadWindow || event->error_code == BadPixmap || event->error_code == BadCursor || event->error_code == BadFont || event->error_code == BadDrawable || event->error_code == BadColor || event->error_code == BadGC || event->error_code == BadIDChoice || event->error_code == BadValue || event->error_code == BadAtom) {
    ...
    snprintf(buffer, BUFSIZ, "+ %s\n", mesg);
    rxvt_warn(buffer, event->resourceid);
  }
}
#endif
...
void sig_handlers::sig_term(ev::sig &w, int revents)
{
  rxvt_emergency_cleanup();
  w.stop();
...
void rxvt_init()
{
  assert(("fontMask must not overlap other RS masks", 0 == (RS_fontMask & (RS_Sel | RS_baseattrMask | RS_customMask | RS_bgMask | RS_fgMask))));
 ...
 */
void rxvt_term::window_calc(unsigned int newwidth, unsigned int newheight)
{
...
bool rxvt_term::set_fonts()
{
  rxvt_fontset *fs = new rxvt_fontset(this);
  rxvt_fontprop prop;

  if (!fs || !fs->populate(rs[Rs_font] ? rs[Rs_font] : "fixed") || !fs->realize_font(1)) {
    delete fs;
    return false;
  }
...
{
  XChangeProperty(dpy, parent, prop, XA_STRING, 8, PropModeReplace, (const unsigned char *)str, len >= 0 ? len : strlen(str));
}

void rxvt_term::set_mbstring_property(Atom prop, const char *str, int len)
{
  XTextProperty ct;
...
void rxvt_term::set_utf8_property(Atom prop, const char *str, int len)
{
  ...
  XChangeProperty(dpy, parent, prop, xa[XA_UTF8_STRING], 8, PropModeReplace, (const unsigned char *)s, strlen(s));
...
void rxvt_term::set_title(const char *str)
{
...
void rxvt_term::set_icon_name(const char *str)
{
...
void rxvt_term::set_window_color(int idx, const char *color)
{
...
bool rxvt_term::set_color(rxvt_color &color, const char *name)
{
...
void rxvt_term::alias_color(int dst, int src)
{
...
get_parent_bw(Display *dpy, Window w)
{
  ...
  XFree(children);
  XGetGeometry(dpy, parent, &wdummy, &idummy, &idummy, &udummy, &udummy, &border_width, &udummy);
...
void rxvt_term::resize_all_windows(unsigned int newwidth, unsigned int newheight, int ignoreparent)
{
  ...
  if (!ignoreparent) {
    ...
    XTranslateCoordinates(dpy, parent, display->root, 0, 0, &x, &y, &unused_cr);
    XGetGeometry(dpy, parent, &unused_cr, &x1, &y1, &unused_w1, &unused_h1, &unused_b1, &unused_d1);
    /*
    ...
    XMoveResizeWindow(dpy, parent, x + dx, y + dy, szHint.width, szHint.height);
#else
 ...
 */
void rxvt_term::set_widthheight(unsigned int newwidth, unsigned int newheight)
{
...
void rxvt_term::im_set_color(unsigned long &fg, unsigned long &bg)
{
  fg = pix_colors[Color_fg];
  bg = pix_colors[Color_bg];
}

void rxvt_term::im_set_size(XRectangle &size)
{
...
void rxvt_term::im_set_preedit_area(XRectangle &preedit_rect, XRectangle &status_rect, const XRectangle &needed_rect)
{
...
void rxvt_term::im_send_spot()
{
  XPoint nspot;
  XVaNestedList preedit_attr;

  if (!Input_Context || !focus || !(input_style & (XIMPreeditPosition | XIMPreeditCallbacks)))
    return;
 ...
 */
bool rxvt_term::im_get_ic(const char *modifiers)
{
  int i, j, found;
  XIM xim;
  XPoint spot;
  ...
  xim_styles = NULL;
  if (XGetIMValues(xim, XNQueryInputStyle, &xim_styles, NULL) || !xim_styles || !xim_styles->count_styles) {
    im_destroy();
    return false;
  }
...
foundpet:
  ...
  if (input_style & (XIMPreeditPosition | XIMPreeditArea)) {
    ...
    sprintf(pat,
            ...
            "-*-*-*-R-*-*-%d-*-*-*-*-*-*,"
            "*",
            fheight, fheight + 1, fheight - 1, fheight - 2, fheight + 2);

    fs = XCreateFontSet(dpy, rs[Rs_imFont] ? rs[Rs_imFont] : pat, &missing_charset_list, &missing_charset_count, &def_string);
  ...
  if (input_style & XIMPreeditPosition) {
    ...
    preedit_attr = XVaCreateNestedList(0, XNForeground, fg, XNBackground, bg, XNArea, &rect, XNSpotLocation, &spot, XNFontSet, fs, NULL);
    ...
    preedit_attr = XVaCreateNestedList(0, XNForeground, fg, XNBackground, bg, XNArea, &rect, XNFontSet, fs, NULL);
    status_attr = XVaCreateNestedList(0, XNForeground, fg, XNBackground, bg, XNArea, &status_rect, XNFontSet, fs, NULL);
  }
    ...
    xcb[0].client_data = (XPointer)this;
    xcb[0].callback = (XIMProc)xim_preedit_start;
    xcb[1].client_data = (XPointer)this;
    xcb[1].callback = (XIMProc)xim_preedit_done;
    xcb[2].client_data = (XPointer)this;
    xcb[2].callback = (XIMProc)xim_preedit_draw;
#if 0
      xcb[3].client_data = (XPointer)this; xcb[3].callback = (XIMProc)xim_preedit_caret;
#endif

    preedit_attr = XVaCreateNestedList(0, XNSpotLocation, &spot, XNPreeditStartCallback, &xcb[0], XNPreeditDoneCallback, &xcb[1], XNPreeditDrawCallback, &xcb[2],
#if 0
  ...
  Input_Context = XCreateIC(xim, XNInputStyle, input_style, XNClientWindow, vt, XNFocusWindow, parent, preedit_attr ? XNPreeditAttributes : NULL, preedit_attr, status_attr ? XNStatusAttributes : NULL, status_attr, NULL);
...
void rxvt_term::im_set_position()
{
  XRectangle preedit_rect, status_rect, *needed_rect;
  XVaNestedList preedit_attr, status_attr;

  if (!Input_Context || !focus || !(input_style & (XIMPreeditArea | XIMPreeditPosition)) || !im_is_running())
    return;
...
void rxvt_term::get_window_origin(int &x, int &y)
{
  Window cr;
